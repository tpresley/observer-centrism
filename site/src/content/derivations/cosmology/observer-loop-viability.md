...
*What is needed to prove or disprove the conjecture:* The geometry functor $G: \mathbf{Obs} \to \mathbf{Geom}$ must be formalized (Gap 5) to the point where the coherence absorption $\Delta c_n$ at each bootstrap level can be computed from the SM particle content and the saturation boundary conditions — without $\Lambda$ as input. If the resulting $\sum \Delta c_n = F(\text{SM structure, saturation conditions})$ yields a unique $\Lambda$ via $\Lambda = 3\pi/((C_0 - F)\ell_P^2)$, the conjecture is proved. If no consistent solution exists, the framework is inconsistent. If multiple solutions exist, additional structure is needed.
...
The framework is consistent with the observed cosmological constant, predicts strict positivity with a computable lower bound, and identifies the remaining task as determining the absolute value of $\Lambda$ from the bootstrap structure (via Conjecture 8.9 and the geometry functor program, Gap 5 below).
