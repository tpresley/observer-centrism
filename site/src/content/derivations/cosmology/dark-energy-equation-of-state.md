...
1. **Quintessence bound**: Theorem 3.1 shows $w = -1$ is the unique fixed point but does not bound $|1 + w|$. A rigorous bound from the Lyapunov correction $\delta\lambda \sim |1 + w| \cdot H$ would require showing that the correction accumulates over cosmic time and eventually destabilizes the loop — giving a maximum $|1 + w|$ consistent with observer persistence over the age of the universe. *Difficulty: HARD.*

   *Attempted route that does not yield a bound (recorded to avoid repetition).* The direct integration of the Lyapunov correction over cosmic time for constant-$w$ quintessence gives $\int_{t_0}^{t_{\text{obs}}} H(t)\, dt = \tfrac{2}{3(1+w)} \ln(t_{\text{obs}}/t_0)$ because $H(t) = 2/[3(1+w) t]$ for $a(t) \propto t^{2/[3(1+w)]}$. The $|1+w|$ prefactor on $\delta\lambda$ cancels against the $1/(1+w)$ in the integrated Hubble rate, giving an integrated Lyapunov drift of order $\tfrac{2}{3}\ln(t_{\text{obs}}/t_0)$ that is **independent of $|1+w|$**. Consequently the direct Lyapunov-accumulation argument does not constrain $|1+w|$ for constant-$w$ quintessence. A rigorous bound likely requires one of: (i) the QEC-threshold connection — quantifying the additional [Substrate Noise](/derivations/foundation/substrate-noise-and-profile-coupling) contribution from non-stationary cosmological geometry and bounding $|1+w|$ below the QEC threshold for observer preservation; (ii) the cross-level consistency of $\Lambda_n^{\text{eff}}$ from [Observer Loop Viability](/derivations/cosmology/observer-loop-viability) Step 8 — requiring the strictly-increasing-with-level sequence (Proposition 8.3) to remain consistent throughout the quintessence evolution; or (iii) a time-varying-$w$ analysis where the Hubble-rate scaling differs from the constant-$w$ form used above. Route (i) looks most tractable but requires developing the Substrate Noise derivation's threshold conditions to include non-stationary backgrounds, which has not been done.

2. **Quantitative $\Lambda$ from horizon distinguishability**: Proposition 4.4 gives bounds on $\Lambda$ that are either Planck-scale (minimal observer) or extremely weak (macroscopic observer). A tighter bound requires computing the minimum non-self distinguishability for the full bootstrap hierarchy — not just one observer, but the network of observers at all scales that the axioms require. This connects to [Observer Loop Viability](/derivations/cosmology/observer-loop-viability) Gap 2 (minimum non-self coherence) and to the Hierarchical Geometry Functor program. *Difficulty: HARD.*
