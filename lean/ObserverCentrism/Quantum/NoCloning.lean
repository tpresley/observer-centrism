/-
  ...
  We formalize this over an arbitrary field (using ℚ for decidability)
  in the simplest setting: ℚ² (a qubit space).
-/
...
/-- Basis state |0⟩ -/
def ket0 : Qubit := fun i => if i = 0 then 1 else 0

/-- Basis state |1⟩ -/
def ket1 : Qubit := fun i => if i = 1 then 1 else 0

/-- Superposition |0⟩ + |1⟩ (unnormalized, which suffices for the argument) -/
def ketPlus : Qubit := fun i => 1
...
/-- The no-cloning theorem: no linear map can clone arbitrary quantum states.
    ...
    Proof sketch: Assume f is a linear cloner.
    - f(|0⟩ + |1⟩) = f(|0⟩) + f(|1⟩)  by linearity
                     = |00⟩ + |11⟩       by cloning on basis states
    - f(|0⟩ + |1⟩) = |+⟩⊗|+⟩            by cloning on |+⟩
                     = |00⟩ + |01⟩ + |10⟩ + |11⟩

    These are different (the |01⟩ and |10⟩ components are 0 vs 1).

    (entanglement Theorem 3.1) -/
theorem no_cloning : ¬ ∃ f : Qubit → TwoQubit, IsCloner f := by
  intro ⟨f, hlin, hclone⟩
  -- Apply linearity: f(|0⟩ + |1⟩) = f(|0⟩) + f(|1⟩)
  have h_linear := hlin ket0 ket1
  -- Apply cloning to basis states
  have h_clone0 := hclone ket0
  have h_clone1 := hclone ket1
  -- Apply cloning to |+⟩ = |0⟩ + |1⟩
  have h_clone_plus := hclone ketPlus
  -- Note: ketPlus = ket0 + ket1 (pointwise)
  have h_plus_eq : (fun i : Fin 2 => ket0 i + ket1 i) = ketPlus := by
    ext i; fin_cases i <;> simp [ket0, ket1, ketPlus]
  -- So f(ketPlus) = f(ket0) + f(ket1) by linearity
  rw [← h_plus_eq] at h_clone_plus
  rw [h_linear] at h_clone_plus
  -- Now h_clone_plus : twoQubitAdd (f ket0) (f ket1) = tensorProduct ketPlus ketPlus
  -- And from cloning: f ket0 = tensorProduct ket0 ket0, f ket1 = tensorProduct ket1 ket1
  rw [h_clone0, h_clone1] at h_clone_plus
  -- Compare component 1 (the |01⟩ component):
  -- LHS: tensorProduct ket0 ket0 [1] + tensorProduct ket1 ket1 [1] = 0 + 0 = 0
  -- RHS: tensorProduct ketPlus ketPlus [1] = 1 * 1 = 1
  have := congrFun h_clone_plus (1 : Fin 4)
  simp [twoQubitAdd, tensorProduct, ket0, ket1, ketPlus] at this
