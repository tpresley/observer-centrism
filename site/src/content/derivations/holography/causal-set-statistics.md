...
**Status.** Heuristic 2.3 is a **framework-specific prediction**, not a generic holographic expectation. Its $\alpha_H = 1/4$ follows from importing the Bekenstein–Hawking count $N_{\text{eff}} = A/(4\ell_P^2)$ into a Planck-tube causal-diamond boundary calculation and summing $\ell_P^2$-variance contributions — equivalently, from treating the holographic count as a count of *fluctuation degrees of freedom* (framework-specific coarse-graining over relational invariants) rather than as an information-capacity cap alone (standard causet-literature position). The two interpretations are numerically distinguishable at the O(1) level:
