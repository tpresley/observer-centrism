...
**Framework coherence note:** The non-associativity argument above is one of three framework-internal confinement arguments that converge on the same conclusion via different mechanisms: (i) Wilson-loop area-law from octonionic non-associativity (this derivation); (ii) QEC-threshold crossing for color-charged codes ([Substrate Noise and Profile-Dependent Coupling Modulation](/derivations/foundation/substrate-noise-and-profile-coupling) Corollary 6.2); (iii) edge-viability failure for isolated color-charged profiles ([Profile-Dependent Edges and Confinement](/derivations/foundation/profile-dependent-edges-and-confinement) Theorem 7.1). Each is structurally distinct; each lands on free quarks not being viable as asymptotic states. Their mutual consistency is a framework-level coherence check.
