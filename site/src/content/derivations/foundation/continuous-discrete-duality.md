...
**(ii) Conjectures 8.9a–8.9b of observer-loop-viability** *(topological vs epoch-conditional bridge): Both endpoints of the bootstrap hierarchy — the Planck observer (discrete UV scale) and the cosmological horizon (continuous IR scale) — are unconditionally gravitationally saturated by framework construction. The bridge between them splits into a topological part (Conjecture 8.9a, SM-determined in principle) and a structural part (Conjecture 8.9b, epoch-conditional and dominant). The duality reframes the bridge question: the topological part is fixed by the discrete layer's division-algebra chain; the structural part requires both the continuous layer's geometry functor and a cosmic-epoch selection principle.*
...
**Remark (Horizons as empirical witnesses of the fixed point).** Horizon coincidences are the framework's clearest concrete instances where the Central Thesis holds exactly. At a Schwarzschild horizon ([Black Hole Entropy](/derivations/holography/black-hole-entropy) Theorem 3.1), the continuous-side area $A = 16\pi G^2 M^2/c^4$ and the discrete-side minimal-loop tile count $N_{\text{bits}} = A/(4\ell_P^2)$ return the same number with full external-observer independence at the same bootstrap level. At a cosmological horizon ([Observer Loop Viability](/derivations/cosmology/observer-loop-viability) Definition 8.2 at level $N$), the same equality holds with full comoving-observer independence. These are the cases currently visible where the two layers' descriptions agree with no observer-dependent slippage — concrete empirical content of the fixed-point claim. Whether the horizon coincidence extends to Kerr and Reissner–Nordström with the same $1/4$ coefficient is open ([Black Hole Entropy](/derivations/holography/black-hole-entropy) Open Gap 3).
