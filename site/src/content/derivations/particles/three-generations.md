...
2. **WKB identification of Theorem 4.2's misalignment-angle tunneling** *(resolved by [Coherence Bounces](/derivations/particles/coherence-bounces) Proposition 2.1)*: The tunneling factors $y_k \sim e^{-\alpha_k/g_{EW}^2}$ are now identified explicitly with $O(4)$-symmetric Euclidean bounce amplitudes in winding-angle configuration space on observer-projected Euclidean spacetime. The WKB form is theorem-level (from Lagrangian semi-classical analysis); specific $\alpha_k$ values remain structural inputs from Theorem 4.2. Coherence Bounces Step 5 confirms the extracted $\alpha_k$ values pass six empirical consistency checks against observed SM Yukawa couplings.
