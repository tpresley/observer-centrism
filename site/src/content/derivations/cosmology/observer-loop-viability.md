...
*What is needed to prove or disprove the conjecture:* The geometry functor $G: \mathbf{Obs} \to \mathbf{Geom}$ must be formalized (Gap 6) to the point where the coherence absorption $\Delta c_n$ at each bootstrap level can be computed as a function of $C_0$ and the known particle content. If the resulting equation $C_0 = f(C_0)$ has a unique fixed point, the conjecture is proved and $\Lambda$ is determined. If the equation has no solution, the framework is inconsistent. If it has multiple solutions, additional structure is needed.
...
**Remark (What Step 8 does and does not show).** Step 8 defines the level-indexed quantities ($S^{(n)}$, $\Lambda_n^{\text{eff}}$), shows where the level-independence approximation breaks down (Proposition 8.5), localizes the hierarchy to the absolute scale of $C_0$ rather than cross-level differences (Proposition 8.7), converts the cosmological constant problem to a self-consistency condition on a finite system (Proposition 8.8), and identifies double saturation as the boundary condition that would close the system (Conjecture 8.9). The hard theorems of this derivation (Theorems 2.1, 4.1, 5.4) remain unchanged — they use the causal horizon, which is ontic and level-independent. The remaining open question is whether the double-saturation fixed-point equation has a unique solution — this is the sharpened form of the geometry functor program (Gap 6).
...
6. **Geometry functor and double saturation**: Step 8 defines the level-indexed quantities ($\Lambda_n^{\text{eff}}$, $S^{(n)}$) and the cross-level consistency constraint (Proposition 8.3). Conjecture 8.9 identifies the boundary condition that would close the system: both the minimal observer (Planck scale) and the cosmological horizon are at gravitational saturation, and the bootstrap hierarchy's determined structure must connect them. A geometry functor $G: \mathbf{Obs} \to \mathbf{Geom}$ must map each observer's epistemic horizon to an effective geometry with its own effective cosmological parameter $\Lambda_n^{\text{eff}}$ (Definition 8.2), compatible with the bootstrap map $\mathcal{R}$ (functoriality across levels). The concrete task: compute the coherence absorption $\Delta c_n$ at each bootstrap level as a function of $C_0$ and the known particle content (division algebra chain, mass hierarchy tunneling factors, coupling ratios), then solve the fixed-point equation $C_0 = S_{\min} + \sum \Delta c_n(C_0) + S_H(C_0)$ for uniqueness. If unique, $\Lambda$ is determined. Requires promoting $\mathcal{R}$ to a full functor on morphisms ([Bootstrap Mechanism](/derivations/interactions/bootstrap), §Remark after Proposition 5.1). Connects to [ER=EPR](/derivations/holography/er-epr) (geometry from relational invariants) and [Renormalization Group](/derivations/thermodynamics-ext/renormalization) (c-theorem, fixed-point structure).
