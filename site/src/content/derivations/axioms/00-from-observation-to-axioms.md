...
Operational content:
- No interaction imports content from outside the universe or exports it to an external reservoir. The ontology is self-contained.
- Any interaction is *between* systems that are already part of the universe — interactions do not create new ontology from nothing or destroy it into nothing.
- Each subsystem carries finite coherence (an observer with compact state space has finite information capacity). Finiteness is a local property of subsystems, not a global constraint on the universe as a whole.
...
**Coherence Conservation** ([Axiom 1](/derivations/axioms/coherence-conservation)). The requirement that subsystems be operationally composable and complementable forces a $\sigma$-algebra of admissible subsystems (Definitions 1 and 2). Closed ontology forces a conserved, positive measure — the coherence measure with its conditions C1–C3 (Definition 4). Positivity (C1) follows because negative coherence has no operational meaning. Local finiteness (C2) follows because each observer has a compact state space with finite information capacity (Definitions 2 and 3 + Axiom 2 O1). The null empty set (C3) is a sanity condition. The requirement that grouping disjoint subsystems cannot create currency forces subadditivity, C4 (Definition 1). The requirement that composite observers contain their sub-observers, combined with the fact that widening a boundary is not an interaction, forces strong subadditivity, C5 (Definitions 5 and 1): a composite's relational coherence with any external system is at least as large as any sub-observer's. The interaction residue being operationally binary forces the dependency graph to be discrete, directed, and acyclic (Definitions 1 and 3), and closed ontology forces local conservation at each node of the graph — coherence in equals coherence out (Definition 4). Conservation across Cauchy slices then follows as a theorem: adjacent slices differ by one node swap, and local conservation at that node preserves the total. All formal elements of this axiom are traced to the operational definitions; no additional input is required.
