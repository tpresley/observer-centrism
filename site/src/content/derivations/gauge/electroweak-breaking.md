...
**Remark (Ledgered-entity bookkeeping across the EWSB transition).** The mode-count is preserved but the ledger-count changes. Pre-EWSB the electroweak sector contains 4 gauge field species ($W^1, W^2, W^3, B$), all unledgered (Type-I quanta of the unbroken gauge group, no rest frame), plus 4 Higgs doublet real scalar modes, none ledgered (the symmetric-phase potential is unstable and the modes have no well-defined Compton period). Post-EWSB the slice presents: 1 photon (off-ledger Type-I quantum, Corollary 7.2); 3 massive vector observers (each ledgered, Proposition 7.1); and the Higgs radial mode, whose ledgered status depends on Loop Closure Axiom 3's $U(1)$ reading (Open Gap 5 below). The broken-phase ledgered-observer count is therefore 3 or 4 depending on the Higgs reading; the symmetric-phase count is zero on either reading. EWSB is the cosmological-scale Type II fusion event that *creates* the electroweak-sector ledgered observers, by analogy to QCD confinement creating the hadronic ledgered observers.
...
5. **Higgs category placement**: The Higgs radial mode has finite Compton period and a single Noether pair (oscillator phase, rest energy), so it satisfies the observer axioms if [Loop Closure](/derivations/axioms/loop-closure) Axiom 3 admits phase-space $U(1)$ as a sufficient loop. Whether Axiom 3 admits the phase-space reading or requires internal/gauge $U(1)$ is currently open; the Higgs is therefore either an elementary scalar observer or a separate non-observer category, with no other consistent placement available. The post-EWSB ledgered-observer count of the electroweak sector (Step 7 above) is correspondingly 3 or 4.
