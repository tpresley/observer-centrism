...
**O2 conditions.** The invariant $I: \Sigma \to V$ must be continuous because discontinuity would mean arbitrarily small state changes causing large identity jumps — the observer's identity would be fragile to noise at every scale, contradicting Definition 2. The target space $V$ must be finite-dimensional because the observer has finite coherence (local finiteness, Axiom 1 C2, on the compact state space $\Sigma$), bounding its information capacity: finitely many bits can support at most finitely many independent conserved quantities, and an infinite-dimensional $V$ would require infinite information to specify the invariant. (When $G_\mathcal{O}$ is later shown to be a Lie group via Axiom 3, the Noether theorem confirms $\dim V = \dim \mathfrak{g}_\mathcal{O}$, consistent with this operational bound.)
