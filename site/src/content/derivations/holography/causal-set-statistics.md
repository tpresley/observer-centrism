...
**Status.** Until a first-principles causet calculation pins down $\alpha_H$, the framework treats it as an O(1) coefficient with $\alpha_H \sim 1/4$ as a suggestive natural value from the holographic substitution. The Holometer experiment ([Holographic Noise](/predictions/holographic-noise)) constrains $\alpha_H \lesssim 0.24$ from its co-aligned ($\beta = 0$) configuration, using the $\gamma(\alpha) = \cos\alpha$ angular structure derived from the relational-invariant nonlocality at the beamsplitter ([Holographic Noise](/predictions/holographic-noise) Theorem 5.1). The natural target $\sim 1/4$ sits in marginal tension with this bound (about 3% above the 95% CL exclusion). Two interpretations are open: (a) the heuristic substitution is correct and the true $\alpha_H$ lies just below $1/4$ at $\sim 0.24$, in which case a Holometer-class experiment with factor-2 sensitivity improvement would directly detect the signal; or (b) the heuristic overestimates by a small constant, and the true $\alpha_H$ is somewhat below $0.24$. Distinguishing these requires the first-principles calculation tracked in [future-targets.json](site/src/data/future-targets.json) (`holographic-noise-amplitude`) — this is the most experimentally consequential open item in the holography chain. $\square$
