...
3. **WKB identification of the seesaw ratio** *(resolved by [Coherence Bounces](/derivations/particles/coherence-bounces) Proposition 4.2)*: The seesaw ratio $M_R/m_D = \exp((\alpha_\nu^{(D)} - \alpha_\nu^{(M)})/g_{EW}^2)$ is now identified explicitly with a ratio of WKB Euclidean bounce actions: level-1 Dirac Yukawa bounce for $m_D$ and level-2 Majorana bounce for $M_R$. Consistency with Theorem 3.1's electroweak-scale estimate requires $\alpha_\nu^{(M)} \lesssim g_{EW}^2 \approx 0.42$ (Majorana pre-aligned at bootstrap level 2), which Coherence Bounces Step 5 Check 6 verifies as empirically consistent.
