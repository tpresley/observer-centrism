...
**(iii) Floor** (minimal-observer era). *Only minimal observers remain. $\mathcal{M} \approx 0$ (no observer above level 0 exists to access the accumulated invariants). But: minimal observers must continue interacting (C5, [Multiplicity](/derivations/minimal-observer/multiplicity), Theorem 7.2). Each interaction generates a new permanent relational invariant ([Relational Invariants](/derivations/interactions/relational-invariants), Theorem 3.2), adding to the already-saturated pair correlation structure. Time continues (phase ticks proceed) but no observer can hold it as memory. The "Floor" here refers to bootstrap level 0 of the observer hierarchy ([Bootstrap Mechanism](/derivations/interactions/bootstrap)) — i.e., Layer 1 of the [three-layer ontology](/derivations/foundation/entity-category-taxonomy) (the structurally minimal observers of [Minimal Observer Structure](/derivations/minimal-observer/structure) Theorem 6.1) — not the framework's Layer 0 substrate, which is the continuous Planck-cell oscillator medium below the observer hierarchy entirely.*
