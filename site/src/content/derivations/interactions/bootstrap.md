...
**Remark (Bootstrap Level 0 sits at Layer 1 of the framework's three-layer ontology).** The "Level 0" of the bootstrap iteration refers to *fundamental observers* — entities satisfying the three axioms with no ledgered constituents. In the three-layer formalization of [Entity Category Taxonomy](/derivations/foundation/entity-category-taxonomy) Step 6, these correspond to Layer 1 entities ($U(1)$ phase oscillators in the abstract minimum, plus Standard Model fundamental particles in their bootstrap-equivalent ground states). The bootstrap iteration does *not* extend below Level 0 to the substrate (Layer 0). Layer 0 sits *below* the bootstrap ladder as the continuous Planck-cell oscillator medium on which the ladder is realized; substrate modes are sub-observer (no $(\Sigma, I, \mathcal{B})$ triple, no loop closure of their own) and are co-created with the Level 0+ hierarchy by the same fixed-point self-consistency that determines the iteration ([Entity Category Taxonomy](/derivations/foundation/entity-category-taxonomy) Theorem 6.2).
...
4. **Fixed-point uniqueness**: Proving Conjecture 7.2 requires a contraction argument or a rigidity result showing the constraints from Axiom 1 (conditions C1–C5) plus the three axioms admit only one self-consistent solution. *Horizon-coincidence equations ([Black Hole Entropy](/derivations/holography/black-hole-entropy) Theorem 3.1, [Observer Loop Viability](/derivations/cosmology/observer-loop-viability) Definition 8.2) provide necessary naturality-like consistency conditions on $\mathcal{U}^*$ — the area-functor and bit-count-functor must satisfy $A = 4\ell_P^2 \cdot N_{\text{bits}}$ at every horizon — but the derived row pair (Schwarzschild + de Sitter) collapses to a single structural equation (saturated holographic bound) and under-determines $\mathcal{U}^*$. Uniqueness cannot be established from horizon consistency alone; the proof requires internal arguments on the category $\mathbf{Obs}$.*
