---
title: "Three Rules and Nothing Else"
subtitle: "The axioms of observer-centrism"
order: 2
openingQuestion: "What is the smallest set of assumptions from which you could derive physics? How compact can a foundation be?"
physicsQuestions:
  - "What are the axioms of observer-centrism?"
  - "Why these three axioms and not others?"
...
Complete honesty requires a caveat. While {{derivationsWithoutPostulatesCount}} of the framework's {{derivationCount}} derivations require nothing beyond these three axioms, the remaining {{derivationsWithPostulatesCount}} require additional assumptions — called [structural postulates](/postulates). These are not arbitrary additions; they specify the mathematical setting (smooth manifolds, symplectic structure, pseudo-Riemannian geometry) in which the derivation operates. They are motivated by the framework's structure but not derived from the three axioms alone.
...
> **On solid ground:** The three axioms are precisely stated, independent, and their sufficiency is tested across {{rigorousCount}} rigorous derivations covering the span from quantum mechanics to general relativity to the information paradox. {{derivationsWithoutPostulatesCount}} derivations require no additional assumptions whatsoever.

> **Work in progress:** The structural postulates — assumptions about smoothness, symplectic structure, Riemannian geometry — are motivated but not derived. Whether they can eventually be derived from the axioms, or whether they represent genuinely independent input, is an open question and an active area of development. Progress is being made: {{nowTheoremCount}} of the {{totalPostulateCount}} original structural postulates have already been [derived as theorems](/postulates) — from the bootstrap mechanism, Weyl averaging, action duality, and uniqueness arguments — reducing the framework's independent assumptions to {{activePostulateCount}}.
