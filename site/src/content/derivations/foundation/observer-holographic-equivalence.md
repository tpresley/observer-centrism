...
**Corollary 4.5 (Exterior cancellation).** *Let $E$ denote any content in the exterior of $\partial M_A$ — whether realized in a neighboring observer's stalk, an extended antipodal patch, or any other structure consistent with the observer-indexed sheaf. Then $A$'s state at tick $t_k$ depends on $E$ only through the integer horizon-crossing record that $E$ induces on $\partial M_A$. Continuous variations of $E$ that leave the integer crossing record invariant produce no change in $A$'s state transitions at any tick.*

*Proof.* By Thesis A (Proposition 2.1), $A$'s state at $t_k$ is determined by the sequenced crossing record $R_\Sigma(A, t_k)$ on any enclosing surface $\Sigma$. Take $\Sigma = \partial M_A$, the outermost enclosing surface (Corollary 4.2). Then $A$'s state at $t_k$ is determined by $R_{\partial M_A}(A, t_k)$. By Proposition 4.1 Part 1, $R_{\partial M_A}$ carries only integer/topological data — winding, linking, framing — with no continuous $U(1)$ content. Two exterior configurations $E, E'$ that induce the same integer crossing record on $\partial M_A$ therefore produce identical $R_{\partial M_A}$ and hence identical $A$-state at $t_k$. Continuous variations of the exterior that preserve the integer crossing residue are operationally null for $A$'s dynamics. $\square$

**Remark 4.5a (The content-split interpretation).** *Corollary 4.5 decomposes exterior content into two components from $A$'s perspective: (i) an integer/topological residue encoded in $R_{\partial M_A}$ — gauge charges, coherence quanta, linking numbers, framings, Chern–Simons levels — which genuinely constrains $A$'s state and is the substance of the framework's horizon-integer descriptions ([Knot-Theoretic Bootstrap](/derivations/foundation/knot-theoretic-bootstrap), [Causal Set Statistics](/derivations/holography/causal-set-statistics)); and (ii) a continuous remainder, comprising all exterior variations compatible with a given integer residue, which is structurally invisible to $A$'s dynamics. The framework's descriptions latch onto (i); component (ii) is the "cancellation" — genuine content that $A$ is structurally incapable of distinguishing from alternative content with the same horizon signature.*

**Remark 4.5b (Stronger than non-recordability).** *The cancellation is stronger than the weaker claim "$A$ cannot record exterior continuous content." It asserts that $A$'s state transitions are outright invariant under continuous variations of the exterior — even conceptually and prospectively. An observer designing an experiment to detect such variations would find no coupling to exploit, because $A$'s dynamics themselves are independent of the canceled component. This is specifically tied to the null character of $\partial M_A$: the analogous claim for a timelike or spacelike enclosing surface fails, since Proposition 4.1 Part 2 preserves continuous phase there.*

**Corollary 4.5bis (Symmetric interior cancellation).** *Let $A$ be any observer and let $B$ be any exterior observer (in a neighboring stalk, or in any other structure consistent with the observer-indexed sheaf) interacting with $A$ through Type III relations that cross $\partial M_A$. Then $B$'s information about $A$ depends on $A$'s interior state only through the integer horizon-crossing record that $A$'s interior induces on $\partial M_A$. Continuous variations of $A$'s interior state that leave the integer crossing record invariant produce no change in $B$-accessible information about $A$.*

*Proof.* Any $A$-$B$ interaction is a Type III relational-carrier exchange that crosses $\partial M_A$ ([Horizon Gauge Shell](/derivations/holography/horizon-gauge-shell) Definition 2.0). The information $B$ obtains about $A$ is the sequenced crossing record $R_{\partial M_A}(A, t_k)$, transmitted outward. By Proposition 4.1 Part 1, $\partial M_A$'s encoding is integer-only: null-generator foliation quotients out continuous $U(1)$ phase content. This is a property of $\partial M_A$'s null geometry, holding regardless of the observer examining the record. Two interior configurations $A, A'$ producing the same integer crossing record $R_{\partial M_A}$ produce identical $B$-observations. $\square$

**Remark 4.5c (Bidirectional exterior cancellation).** *Corollary 4.5 and Corollary 4.5bis are the two sides of a single bidirectional cancellation structure: the same null-generator phase-discard mechanism (Proposition 4.1) that makes exterior continuous content invisible to $A$ (Corollary 4.5) makes $A$'s interior continuous content invisible to any $B$ outside $\partial M_A$ (Corollary 4.5bis). Both directions are structurally forced by the null character of $\partial M_A$ alone, with no additional commitments. The symmetric pair is the basis for the mutual opacity theorem of [Observer Edges and Mutual Opacity](/derivations/foundation/observer-edges-and-mutual-opacity) — same-level observers beyond their mutual edge communicate exclusively through integer surface residues, with neither observer accessing the other's continuous interior.*
...
- **Corollary 4.5 (exterior cancellation)**: direct consequence of Thesis A applied to $\partial M_A$ as the outermost enclosing surface, combined with Proposition 4.1's integer-only characterization. Continuous exterior content compatible with a given integer horizon residue is operationally null for $A$'s state transitions — stronger than mere non-recordability.
- **Corollary 4.5bis (symmetric interior cancellation)**: the dual of Corollary 4.5 via the same Proposition 4.1 mechanism. Exterior observer $B$'s access to $A$'s interior is quotiented by the null-generator foliation of $\partial M_A$, leaving only integer horizon-crossing data. This is the basis for the mutual opacity theorem of [Observer Edges and Mutual Opacity](/derivations/foundation/observer-edges-and-mutual-opacity).
- **Corollary 4.6 (operational completeness of the sheaf)**: universal application of Corollary 4.5 and 4.5bis across all observers. Establishes that the framework's manifold-with-boundary stalks suffice to capture all testable content; any hypothetical geometric extension is operationally invisible.
