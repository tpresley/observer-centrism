...
**Note on substrate ontology.** The "substrate" here is the framework's Layer 0 of the three-layer ontology formalized in [Entity Category Taxonomy](/derivations/foundation/entity-category-taxonomy) Step 6: continuous Planck-cell oscillator modes that are *not themselves observers* (no $(\Sigma, I, \mathcal{B})$ triple, no loop closure of their own) but constitute the sub-observer medium on which the Layer 1+ observer hierarchy is realized. Layer 0 is co-created with the observer network: the variance $\alpha_H$ used below is forced by the bootstrap fixed-point self-consistency that determines the Layer 1+ hierarchy ([Entity Category Taxonomy](/derivations/foundation/entity-category-taxonomy) Theorem 6.2; [Holographic Noise](/predictions/holographic-noise); [Area Scaling](/derivations/holography/area-scaling)) rather than postulated as an independent free parameter. The noise is the network's internal resolution limit at sub-observer scales, not an external bath imposed on the network from outside.
