...
2. **Quantum electrodynamics**: This derivation gives classical Maxwell equations. The quantized theory (QED) requires applying the [Born Rule](/derivations/quantum/born-rule) to the gauge field: the photon is the quantum of $F_{\mu\nu}$, and Feynman diagrams describe coherence exchanges between charged observers mediated by virtual photons. In the framework's ontological taxonomy ([Entity Category Taxonomy](/derivations/foundation/entity-category-taxonomy)), the photon is the canonical {Type-I quantum, Internal-charge-carrier} entity: it is the quantized phase-transfer carrier of the $U(1)_{em}$ connection $A_\mu$ derived above ([Three Interaction Types](/derivations/interactions/three-types) Definition 4.1, Proposition 4.2), propagating between charged observers along the null trajectory $L = cT$ ([Speed of Light](/derivations/spacetime/speed-of-light) Proposition 6.2). It is off-ledger — the connection $A_\mu$ implements phase comparison between ledgered observers, and the photon is its quantized vacuum wave solution, not itself a ledgered observer in $(\Sigma, I, \mathcal{B})$ form.
...
2. **Magnetic monopoles** — *Resolved by structural exclusion*: The gauge potential is not an optional convenience but the mechanism of phase comparison between observers, so it must exist globally. The Bianchi identity then gives $\nabla \cdot \mathbf{B} = 0$ necessarily. See the [No Magnetic Monopoles](/predictions/no-magnetic-monopoles) prediction.

3. **Non-abelian extension** — *Resolved by [Weak Interaction](/derivations/gauge/weak-interaction) and [Color Force](/derivations/gauge/color-force) (both rigorous)*: The template established here (local symmetry → connection → curvature → uniqueness → field equations) has been successfully extended to $SU(2)$ and $SU(3)$ through the division algebra hierarchy $\mathbb{R} \to \mathbb{C} \to \mathbb{H} \to \mathbb{O}$.
