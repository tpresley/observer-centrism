...
**Note on non-AdS extension.** The standard formulation of ER=EPR (Maldacena–Susskind 2013) lives in AdS/CFT, where Ryu–Takayanagi supplies the area–entropy identity directly. The argument below is intended to hold *off* AdS as well — for two entangled observers in (e.g.) asymptotically flat space — by replacing RT with the framework's area-scaling result. This non-AdS extension carries three conditionalities that the AdS version does not, all of which are now addressed in the body: dimensional accounting (Remark 3.1), discrete/continuum boundary (Section 3.4), and handle vs. flux-tube topology (Theorem 3.2 Step 2, via the [Channel Irreducibility and the Discrete Handle](/derivations/holography/channel-irreducibility-and-discrete-handle) lemma as of 2026-05-27). The handle lemma is itself at `draft` rigor — see its Open Gaps section for what remains to upgrade it to `provisional`. The most important content for downstream usage — that the flux-tube reading is *excluded* (not merely "one of two possible readings") — is at full `derived` rigor (lemma Theorem 4.4).
...
| Theorem 3.2 (wormhole geometry) | Derived modulo lemma open gaps | Step 1 (causal-set topology) and Step 3 (minimal surface) as before. Step 2 (handle topology in the continuum) now rests on the [Channel Irreducibility and the Discrete Handle](/derivations/holography/channel-irreducibility-and-discrete-handle) lemma, Corollary 4.6. The lemma's own open gaps (quantitative sprinkling match; antichain inextendibility) are minor; the flux-tube exclusion (lemma Theorem 4.4) is at full derived rigor. Reading is now the bona-fide ER-bridge handle topology, not flux tube |
...
| Theorem 5.1 (exact correspondence) | Derived modulo lemma open gaps | Rows 1–4 established by rigorous upstream derivations. The geometric side is now handle topology (not flux tube) via the [Channel Irreducibility and the Discrete Handle](/derivations/holography/channel-irreducibility-and-discrete-handle) lemma. Row 5 (Schmidt ↔ quasi-normal mode) is conjectural (see Gap 1) |
...
**Gap 3 (flux tube vs. spatial handle — load-bearing for non-AdS extension).** *Status updated 2026-05-27: closed (draft level) by the [Channel Irreducibility and the Discrete Handle](/derivations/holography/channel-irreducibility-and-discrete-handle) lemma.* The lemma's four-step program (define ambient embedding of a sub-causal-set; show embedding ⇒ decoherence; contrapose to show strict irreducibility ⇒ non-embedding; combine an explicit Morris–Thorne sprinkling existence argument with a flat-space exclusion argument via Major–Rideout–Surya 2007) establishes that the ambient Cauchy slice acquires a non-contractible 1-cycle as a result of channel irreducibility. The lemma's own remaining open gaps (quantitative sprinkling-density match for the wormhole construction; inextendibility of the spanning antichain; deferred connection to the Hauptvermutung for a uniqueness upgrade) are minor by comparison and do not affect the qualitative handle vs. flux-tube distinction. Closure is at `draft` status; promotion to `provisional` depends on closing the lemma's Gap 1 (quantitative match) and Gap 2 (spanning antichain). The flux-tube reading is excluded outright (lemma Theorem 4.4) — this part is at full `derived` rigor and is the most important content for downstream usage.
...
- [Entanglement](/derivations/quantum/entanglement), Gap: "Make the ER=EPR sketch (Proposition 5.1) rigorous by connecting to the Area Scaling and Causal Set Statistics derivations" — **Resolved at draft level**: Full derivation with dual descriptions, causal set topology argument via rigorous Causal Set Statistics, and area-entropy equality. In AdS settings the resolution is essentially complete modulo Gap 1 (Schmidt ↔ quasi-normal mode). In non-AdS settings the geometric side is now handle topology (not flux tube), via the [Channel Irreducibility and the Discrete Handle](/derivations/holography/channel-irreducibility-and-discrete-handle) lemma which closes the former Gap 3.
