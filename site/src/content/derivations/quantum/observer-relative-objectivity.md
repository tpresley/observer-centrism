...
2. **Contextuality connection**: The Kochen-Specker theorem shows that non-contextual hidden variable models are impossible. This is closely related to Theorem 5.1 (no universal definiteness) but approaches from a different direction. The [Sheaf Structure](/derivations/quantum/sheaf-section-uniqueness) derivation identifies this connection: Level 2 facts are precisely the local sections of the outcome sheaf that fail to globalize, and Kochen-Specker contextuality is the cohomological obstruction ($\check{H}^1 \neq 0$). What remains is a formal isomorphism with the Abramsky-Brandenburger sheaf-theoretic contextuality framework — mapping relational invariants to measurement contexts and showing the two cohomologies coincide.
...
5. **Sheaf structure — remaining quantitative questions**: The [Sheaf Structure](/derivations/quantum/sheaf-section-uniqueness) derivation resolves the qualitative question: the observer network carries three sheaves (coherence, probability, outcome) with a split answer. The coherence and probability sheaves have unique global sections (C2 + Born rule uniqueness), while the outcome sheaf does not ($\check{H}^1 \neq 0$ from Kochen-Specker contextuality). Temporal branching is ruled out (DAG acyclicity → contractible nerve → $H^1_{\text{temporal}} = 0$). What remains open is *quantitative*: is $\dim \check{H}^1$ related to a physical quantity (entanglement entropy, mutual information, number of non-commuting observables)? And does $H^2$ or higher cohomology carry physical meaning — perhaps related to higher-order contextuality or the structure of entanglement networks?
