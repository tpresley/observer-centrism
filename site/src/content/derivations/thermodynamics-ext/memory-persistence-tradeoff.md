...
### Step 5: Type II Fusion Pauses Constituent Clocks

The memory budget $\dot{\mathcal{C}}_{\text{III}}$ entering Theorem 4.1 is computed against an observer that is itself on the Cauchy-slice ledger. A constituent of a Type II composite is not. By [Three Interaction Types](/derivations/interactions/three-types) Definition 1.1, an interaction is a map $T_{12}: \Sigma_1 \times \Sigma_2 \to \Sigma_1 \times \Sigma_2$ between two observer state spaces. A constituent absorbed into a fused composite no longer has a state space on the slice — its degrees of freedom have been pushed through the fusion projection $\pi: \Sigma_1 \times \Sigma_2 \to \Sigma_{12}$ ([Three Interaction Types](/derivations/interactions/three-types) Definition 4.3). External Type III interactions can only target what remains: the composite. This has a sharp consequence for the memory budget of the constituent.

**Theorem 5.1 (Type II clock-pause).** *Let $\mathcal{O}_C = (\Sigma_C, I_C, \mathcal{B}_C)$ be a Type II composite formed by Fusion of constituents $\mathcal{O}_1, \mathcal{O}_2, \ldots, \mathcal{O}_n$. For as long as $\mathcal{O}_C$ persists on the Cauchy slice:*

*(i) (External Type III interactions cannot reach a constituent.) For any external observer $\mathcal{O}_X$ on the slice, every Type III interaction is a map $T_{XC}: \Sigma_X \times \Sigma_C \to \Sigma_X \times \Sigma_C$ on the composite's state space, generating a relational invariant $I_{XC}$ on $\Sigma_X \times \Sigma_C$. No Type III map $T_{Xk}: \Sigma_X \times \Sigma_k \to \Sigma_X \times \Sigma_k$ for $k \in \{1, \ldots, n\}$ is admissible.*

*(ii) (Type III accumulation clock is paused.) The Type III memory accumulation rate $\dot{\mathcal{C}}_{\text{III}}(\mathcal{O}_k)$ for each constituent is identically zero on the interval of fusion. Memory accumulation is rerouted entirely onto the composite's budget $\dot{\mathcal{C}}_{\text{III}}(\mathcal{O}_C)$.*

*(iii) (Re-emergence with paused clock state.) On dissolution of $\mathcal{O}_C$ — by decay (reverse Type II, [Three Interaction Types](/derivations/interactions/three-types) Definition 7.2) into product observers including some $\mathcal{O}_k'$ — each released constituent re-enters the ledger with its individual Type III state-space content unchanged from the moment of fusion. Its accumulated perturbation $\sum_i \mathcal{C}(I_i)$ relative to its own loop is identical before fusion and after release.*

*Proof.* (i) An interaction is, by Definition 1.1 of [Three Interaction Types](/derivations/interactions/three-types), a map between observer state spaces. After Fusion, $\Sigma_1, \ldots, \Sigma_n$ are no longer observer state spaces on the slice — they have been replaced by the single composite state space $\Sigma_C$ via the projection $\pi$ (Definition 4.3 of three-types). $\Sigma_C$ is the only domain on the constituent side that an admissible interaction map can target. Any putative $T_{Xk}$ would require $\Sigma_k$ to exist on the slice as an independent factor, contradicting the dimension-reduction criterion of Type II ($\dim \Sigma_C < \sum_k \dim \Sigma_k$).

(ii) Memory accumulation (Proposition 2.3) is generated only by Type III interactions targeting the observer in question. By (i), no such interaction targets $\mathcal{O}_k$ during fusion. Therefore $\dot{\mathcal{C}}_{\text{III}}(\mathcal{O}_k) = 0$. The composite, by contrast, is a fully ledgered observer; its Type III interactions with $\mathcal{O}_X$ contribute to $\dot{\mathcal{C}}_{\text{III}}(\mathcal{O}_C)$, perturbing the composite's loop closure (Proposition 2.1) on its expanded $\Sigma_C$.

(iii) On decay, the dimension-decreasing projection $\pi$ is reversed (Definition 7.2 of three-types). The product structure is restored on the slice: $\Sigma_C \to \Sigma_1' \times \cdots \times \Sigma_n'$. Each released $\mathcal{O}_k'$ inherits the constituent's pre-fusion state-space content modulo the redistribution of binding coherence (Proposition 7.3 of three-types). The Type III invariants $\{I_i\}$ that $\mathcal{O}_k$ had absorbed prior to fusion are properties of $\Sigma_k$ that survived the projection $\pi$ (they were the structural content of $\Sigma_k$ at fusion); the composite's external invariants $I_{XC}$ are properties of $\Sigma_C$ that distribute across products and emitted radiation per Proposition 7.3. The constituent's individual ledger is recovered intact. $\square$

**Corollary 5.2 (Composite-level decay channel replaces constituent decay channel).** *If $\mathcal{O}_k$ had a Type II reverse channel available as a free observer (an internal instability — would-be self-decay), this channel is replaced during fusion by the Type II reverse channel of $\mathcal{O}_C$. The composite's decay channels are determined by the composite's own bootstrap fixed point structure, not by the sum of constituent instabilities.*

*Proof.* A Type II reverse channel for $\mathcal{O}_k$ is a map $\Sigma_k \to \Sigma_{k_1} \times \Sigma_{k_2}$ that increases dimension on $\mathcal{O}_k$'s state space (Definition 7.2 of three-types). During fusion, $\Sigma_k$ does not exist as an independent factor; the only state space available for dimension-increasing maps is $\Sigma_C$. Therefore the only admissible Type II reverse channels are those defined on $\Sigma_C$. A bound electron in a hydrogen atom does not undergo electron decay through any independent channel, even if the free electron had one — the only decay channels available to the system are the atom's. $\square$

**Remark (Stable composites pause both clocks).** A Type II composite at an exact bootstrap fixed point has $\epsilon = 0$: no internal instability clock ticks (Proposition 7.7 of three-types is not engaged), and no external Type III memory accumulates against any constituent (Theorem 5.1). The constituents are doubly protected. An unstable composite (composite ε > 0, finite lifetime) still pauses the constituent's external Type III clock for the duration of fusion — only the composite's own internal-instability clock ticks.

**Remark (Why matter persists).** The dominant fact about the present epoch is that the vast majority of elementary fermions are not free. Quarks are confined within hadrons (Type II fusion at the strong scale, [Confinement](/derivations/gauge/confinement)); nucleons are bound in nuclei (Type II fusion at the nuclear scale); electrons are bound in atoms (Type II fusion at the atomic scale); atoms are bound in molecules and condensed matter. Each of these binding levels pauses the Type III clock of the constituent at the level immediately below. A free quark, were it to exist, would have a Type III memory budget independently exposed to every QCD-scale interaction in the environment; inside a proton, the quark's clock is paused, and only the proton's clock counts against the proton's $\mathcal{K}$. The hierarchy of nested Type II composites is not incidental — it is the framework's mechanism for protecting fermion content over cosmological timescales. The memory-persistence tradeoff bites only at the topmost unbound layer.

**Worked example (Hydrogen atom).** Consider a free electron $e^-$ in the CMB bath. By Proposition 2.3 and the consistency check below (Electron entry), its Type III interaction rate is dominated by Compton scattering above the inelastic threshold $\hbar\omega \gtrsim m_e c^2$ — vanishingly rare in the CMB bath, but nonzero. The clock $\dot{\mathcal{C}}_{\text{III}}(e^-)$ ticks slowly but it ticks.

The electron then binds with a free proton to form a hydrogen atom (Type II fusion: $\Sigma_e \times \Sigma_p \to \Sigma_H$, with $\dim \Sigma_H < \dim \Sigma_e + \dim \Sigma_p$). By Theorem 5.1(ii), $\dot{\mathcal{C}}_{\text{III}}(e^-) = 0$ for the duration of binding. The hydrogen atom now has its own Type III clock $\dot{\mathcal{C}}_{\text{III}}(H)$, dominated at low energies by Lyman-alpha resonance and at higher energies by ionization. By Corollary 5.2, the electron's would-be free decay channels (none, in the standard model) are replaced by the atom's: spontaneous emission, ionization, hyperfine transitions.

If the atom is then ionized by a UV photon (Type II reverse: $\Sigma_H \to \Sigma_e \times \Sigma_p$), the released $e^-$ re-enters the ledger by Theorem 5.1(iii) with its individual Type III state-space content identical to its pre-fusion value. The clock resumes ticking from where it paused. The 13.6 eV of binding coherence is carried away by the released photon and the kinetic energy of the products (Proposition 7.3 of three-types).

The hydrogen atom's lifetime against memory-driven dissolution is set by $\mathcal{K}_H / \dot{\mathcal{C}}_{\text{III}}(H)$, not by anything about the electron's individual budget. This is why neutral hydrogen is the most abundant baryonic structure in the universe — both the electron and the proton are riding the atom's clock, and the atom's clock is slow.
