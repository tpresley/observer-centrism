...
**Remark (W and Z are Type II composites; Higgs category placement is open).** The W and Z are not elementary observers in the framework's bootstrap hierarchy but Type II composites in the sense of Definition 4.3 and the ledgered-observer-count reading above. The fusion event is electroweak symmetry breaking: each massive vector observer is the result of one transverse pre-EWSB gauge mode combining with one Goldstone scalar mode of the Higgs doublet ([Electroweak Symmetry Breaking](/derivations/gauge/electroweak-breaking) Proposition 2.2; the Goldstone equivalence theorem identifies the longitudinal polarization with the absorbed Goldstone). The cycle count $N_{\text{cycles}} \sim 6$ then reads as the lifetime of the fused composite against its reverse Type II channel — decay back to lighter fermion pairs — measured against its own Compton period; the channel barrier is essentially absent at the EW scale, so the floor sits a single order of magnitude above the loop period. The Higgs's category placement is currently open: the radial mode of the Higgs doublet has finite Compton period and a single Noether pair (oscillator phase, rest energy), so it satisfies the observer axioms if Loop Closure ([Loop Closure](/derivations/axioms/loop-closure) Axiom 3) admits phase-space $U(1)$ as a sufficient loop, and falls into a separate category if Axiom 3 requires internal/gauge $U(1)$. Both readings are currently consistent with the framework; the resolution is an open question. The cycle-count metric records the Higgs's persistence regime ($\sim 5{,}000$ cycles) without committing to either reading.
