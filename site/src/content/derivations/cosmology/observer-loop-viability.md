...
summary: "The three axioms constrain which spacetimes can host observer networks. The Planck-scale upper bound (Λ < 3/ℓ_P²) follows from geometric viability. Strict positivity (Λ > 0) follows from coherence conservation on two fronts: a Λ < 0 bounce destroys all observer structures via divergent effective pressure, and Λ = 0 would require infinite per-observer coherence budget, contradicting Cauchy-slice integer quantization and the holographic bound on each observer's epistemic horizon."
rigorLevel: "formal"
...
where $\Delta c_n > 0$ is the coherence absorbed at level $n$. The number of levels resolvable within a given observer's projected geometry is bounded by the holographic information capacity of that geometry ([Bootstrap Mechanism](/derivations/interactions/bootstrap), Proposition 6.2). $\square$
...
At the bottom of the hierarchy, the minimal observer has spatial extent $\sim \ell_P$, Schwarzschild radius $R_S(m_P) = 2Gm_P/c^2 \sim \ell_P$, and Compton wavelength $\lambda_C(m_P) = \hbar/(m_P c) \sim \ell_P$. All three scales coincide by construction ([Minimal Observer Structure](/derivations/minimal-observer/structure), Proposition 7.1). The holographic bound is saturated automatically: the observer's boundary area $A_0 \sim \ell_P^2$ encodes exactly $\sim 1$ bit, and the observer is simultaneously the smallest possible and already at its own gravitational collapse limit.
...
[Mass Hierarchy](/derivations/particles/mass-hierarchy) (Step 7) shows that the bridge decomposes into two regimes with different information scaling:
...
**Theorem 8.10 (Strict positivity of $\Lambda$).** *$\Lambda > 0$, with lower bound*
$$\Lambda \geq \frac{3\pi}{C_0^{(N)}\,\ell_P^2}$$
*where $C_0^{(N)}$ is the per-observer Cauchy-slice total at the highest bootstrap level $N$.*

*Proof.* By [Coherence Conservation](/derivations/axioms/coherence-conservation) Corollary 5.5a, the Cauchy-slice total is an integer multiple of $\hbar\omega_0$: $C_0^{(n)} = N^{(n)}\,\hbar\omega_0$ with $N^{(n)} \in \mathbb{Z}_{\geq 0}$. By Proposition 7.4 of the present derivation, $C_0$ is a per-observer quantity: each observer's budget is determined by its own epistemic horizon with boundary area $A_\mathcal{B}$, bounded above by the holographic capacity $A_\mathcal{B}/(4\ell_P^2)$. Per-observer finiteness therefore applies universally — no observer has infinite accessible coherence, regardless of the global spatial extent of the interaction graph.

Definition 8.1 decomposes $C_0 = \mathcal{C}_{\text{acc}}^{(n)} + S^{(n)}$ with both components non-negative, so $S^{(n)} \leq C_0^{(n)}$. Definition 8.2 gives $\Lambda_n^{\text{eff}} = 3\pi/(S^{(n)}\ell_P^2)$. Combining:
$$\Lambda_n^{\text{eff}} \;\geq\; \frac{3\pi}{C_0^{(n)}\,\ell_P^2} \;>\; 0$$
at every bootstrap level. At the highest level $N$, $S^{(N)} = S_H$ and $\Lambda_N^{\text{eff}} = \Lambda$ — the cosmological constant in the Einstein field equations. Combined with Theorem 5.4 ($\Lambda \geq 0$), strict positivity $\Lambda > 0$ follows. $\square$

**Corollary 8.11 (Numerical lower bound).** *Using the per-observer coherence budget $C_0 \approx 4.6 \times 10^{122}\,\hbar\omega_0$ of Proposition 8.7, the framework-derived lower bound is*
$$\Lambda \;\gtrsim\; 2 \times 10^{-122}\,\ell_P^{-2}$$
*— coinciding with the level-0 projection $\Lambda_0^{\text{eff}}$ of Proposition 8.7. The observed value $\Lambda_{\text{obs}} \approx 2.9 \times 10^{-122}\,\ell_P^{-2}$ lies at factor $\sim 1/\Omega_\Lambda \approx 1.43$ above this bound, consistent with measurement at bootstrap level $N$ where $S^{(N)} < C_0$.*

**Remark (connection to the cosmological arrow of time).** The structural arrow of [Time as Phase Ordering](/derivations/thermodynamics/time) (Theorem 6.1) is monotone non-decreasing relational invariant depth along any directed path. At cosmological scale, Theorem 8.10 forces $\Lambda > 0$, which via Einstein's equations gives eternal expansion — the Gibbons-Hawking bath of the resulting de Sitter phase supplies persistent event generation at constant rate per Hubble volume. Matter-dominated $\Lambda = 0$ cosmologies, by contrast, give a total event count per comoving volume that converges as $t \to \infty$ (integrated two-body interaction rate $\propto t^{-2}$), so the structural arrow saturates in finite depth. Strict positivity of $\Lambda$ therefore identifies the global cosmological arrow (eternal expansion) with the structural arrow (unbounded $d(v)$) at the level of the framework's coherence accounting.
...
| Status | Requires string landscape (unproven) | Requires geometry functor (Gap 5, open) to compute bridge and solve fixed-point equation |
...
- Theorem 5.4 (sign prediction, $\Lambda \geq 0$): follows rigorously from Theorem 5.2 combined with standard cosmology ($\Lambda < 0$ implies recollapse to Planck density).
- Theorem 8.10 (strict positivity, $\Lambda > 0$): follows from Cauchy-slice integer quantization ([Coherence Conservation](/derivations/axioms/coherence-conservation) Corollary 5.5a), per-observer budget structure (Proposition 7.4), holographic bound on the observer's epistemic horizon (Proposition 1.5), and the level-indexed relation of Definition 8.2. The chain is elementary algebra once the inputs are in place.
- Corollary 8.11 (numerical lower bound, $\Lambda \gtrsim 2 \times 10^{-122}\,\ell_P^{-2}$): direct substitution of the per-observer budget from Proposition 8.7 into Theorem 8.10's bound.
...
- Proposition 3.1 (holographic budget): uses the holographic entropy bound (derived) applied to the de Sitter horizon. The application to the multiplicity requirement involves interpreting the bound as a coherence budget (the de Sitter horizon is static in the static patch, so the bound applies, but the connection to the coherence budget for interior observers involves assumptions from [Entropy](/derivations/thermodynamics/entropy) not explicitly formalized for cosmological horizons). This is an independent confirmation of the Planck-scale bound, not in the critical path.
- Proposition 5.3 (Type II fusion reinforcement): the gravitational fusion dynamics are well-established via the information paradox resolution (derived), but the claim that the approach to $\rho_P$ goes "entirely through fusion" is a physical argument. This reinforces the sign prediction but is not required for it.
...
**Conjectural / open:**
- Proposition 6.2 (conditions for tighter upper bound): the key open question for sub-Planck tightening of the upper bound $\Lambda < 3/\ell_P^2$. No mechanism is known within the framework to force $\mathcal{C}_{\min}^{\text{non-self}} \gg 1$.
- Step 7 (hierarchical coherence suppression): a mechanism sketch connecting bootstrap, c-theorem, and ER=EPR. The quantitative prediction of $\Lambda$ (not just the lower bound) requires the geometry functor and knowledge of $C_0$.
- Conjectures 8.9a–8.9b (topological vs epoch-conditional bridge): split the earlier "double saturation uniquely determines $\Lambda$" conjecture, which is excluded by the epoch-dependence of the structural regime. 8.9a (topological bridge SM-determined, $O(10^2)$ bits) awaits explicit enumeration of topological information content at each algebra level. 8.9b (structural bridge requires epoch selection) requires both the geometry functor and a framework-intrinsic cosmic-epoch selection principle; neither is in place.

**Assessment:** Rigorous. The core results form a rigorous chain: the geometric upper bound (Theorem 2.1) follows from the Planck cutoff and de Sitter geometry; bounce dissolution (Theorem 4.1) follows from divergent effective pressure exceeding any finite observer energy; the lower sign bound (Theorem 5.4) follows from coherence conservation excluding the bounce; strict positivity (Theorem 8.10) follows from integer quantization of the per-observer coherence budget combined with the level-indexed cosmological parameter. Together these give $0 < \Lambda < 3/\ell_P^2$, with explicit lower bound $\Lambda \gtrsim 3\pi/(C_0\ell_P^2)$ (Corollary 8.11) within a factor of $1/\Omega_\Lambda \approx 1.43$ of the observed value — a cosmic-evolution factor rather than a framework-fundamental ratio (see remark following Conjecture 8.9b). The remaining open questions are the absolute value of $C_0$ (Conjectures 8.9a–8.9b: topological bridge enumeration and cosmic-epoch selection, plus the geometry functor program) and the coincidence problem (why $\Omega_m \sim \Omega_\Lambda$ now), which Conjecture 8.9b subsumes.
...
| [Time as Phase Ordering](/derivations/thermodynamics/time) | Partial order on interaction graph (Thm 4.2), no observers → no time (Prop 8.1), structural arrow (Thm 6.1) |
| [Bootstrap Mechanism](/derivations/interactions/bootstrap) | Hierarchy levels (Cor 2.2), necessity (Thm 3.1), irreducibility (Cor 4.2), information ceiling (Prop 6.2), bootstrap map $\mathcal{R}$ (Prop 5.1) |
...
- **Strict positivity**: Achieved. The axioms predict $\Lambda > 0$ with a framework-computable lower bound. The lower sign bound ($\Lambda \geq 0$, Theorem 5.4) follows from bounce dissolution; the strict inequality ($\Lambda \neq 0$, Theorem 8.10) follows from per-observer holographic finiteness combined with the level-indexed cosmological parameter. Corollary 8.11 gives $\Lambda \geq 3\pi/(C_0\ell_P^2) \approx 2 \times 10^{-122}\,\ell_P^{-2}$.
