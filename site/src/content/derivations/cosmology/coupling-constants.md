...
- Corollary 4.2: No GUT proton decay — logical consequence of the no-GUT result in [Standard Model Gauge Group](/derivations/gauge/standard-model-group) Theorem 3.1.
...
- Theorem 4.1: Non-convergence of couplings — the "generically inconsistent" step in argument (2) is heuristic. The no-GUT conclusion survives via the independent [Standard Model Gauge Group](/derivations/gauge/standard-model-group) Theorem 3.1 (no SU(5)/SO(10) embedding from the axioms), so the corollary Sect. 4.2 follows; but the coupling-convergence argument in this derivation is not a formal proof and should either be rebuilt or explicitly delegated.
- The jump from "1 of 3 imaginary quaternionic directions" to $\sin^2\theta_W = 1/3$ in Step 1 part (2) is now made via the S1 ratio (consistent arithmetic), but the framework-native justification for why $\alpha_i \propto \dim \mathbb{A}_i$ (as opposed to some other scaling) is left to the structural postulate.
