...
**Gap 3 (flux tube vs. spatial handle — load-bearing for non-AdS extension).** *Status updated 2026-05-27: closed (draft level) by the [Channel Irreducibility and the Discrete Handle](/derivations/holography/channel-irreducibility-and-discrete-handle) lemma.* The lemma's four-step program (define ambient embedding of a sub-causal-set; show embedding ⇒ decoherence; contrapose to show strict irreducibility ⇒ non-embedding; combine an explicit Morris–Thorne sprinkling existence argument with a flat-space exclusion argument via Major–Rideout–Surya 2007, with the spanning antichain construction supplied by Bernal–Sánchez 2003) establishes that the ambient Cauchy slice acquires a non-contractible 1-cycle as a result of channel irreducibility. The lemma's only remaining open gap (quantitative sprinkling-density match for the wormhole construction) does not affect the qualitative handle vs. flux-tube distinction. Closure is at `draft` status; promotion to `provisional` depends on closing the lemma's Gap 1 (quantitative match). The flux-tube reading is excluded outright (lemma Theorem 4.4) — this part is at full `derived` rigor and is the most important content for downstream usage.
